...
def build_dataset(args):
    input = Path(args.input_path).read_text().split("\n")
    input = [smi for smi in input if smi != "" and smi is not None]

    target = Path(args.target_path).read_text().split("\n")
    target = [smi for smi in target if smi != "" and smi is not None]

    return ReactionDataset(input, target)
...
def predict(model, test_loader):
    device = "cuda:1" if util.use_gpu else "cpu"
...
def write_predictions(args, smiles, log_lhs, original_smiles):
    ...
    df.to_pickle(Path(args.output_path))
